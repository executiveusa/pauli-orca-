...
const SANDBOX_ROOT = path.resolve(process.env.PAULI_SANDBOX_ROOT || '/workspaces/pauli')
...
async function probeCapabilities() {
  const receipts = []
  receipts.push(await run(GIT_BIN, ['--version']))
  receipts.push(await run(ORCA_BIN, ['agent-context', '--json']))
  receipts.push(await run(ORCA_BIN, ['status', '--json']))
  const failures = receipts.filter((receipt) => receipt.exitCode !== 0).map((receipt) => `${receipt.command.slice(0, 2).join(' ')} exited ${receipt.exitCode}`)
...
async function prepareJob(job) {
  ...
  if (!repoDir.startsWith(`${SANDBOX_ROOT}${path.sep}`)) throw new Error('sandbox path escaped configured root')
  ...
  try {
    ...
    try {
      ...
      const fetchReceipt = await run(GIT_BIN, ['fetch', '--prune', 'origin'], { cwd: repoDir })
      receipt.commandReceipts.push(fetchReceipt)
      if (fetchReceipt.exitCode !== 0) throw new Error('git fetch failed')
    } catch (error) {
      if (error?.message === 'git fetch failed') throw error
      await fs.rm(repoDir, { recursive: true, force: true })
      const cloneReceipt = await run(GIT_BIN, ['clone', '--filter=blob:none', '--no-tags', `https://github.com/${job.repository}.git`, repoDir])
      receipt.commandReceipts.push(cloneReceipt)
      if (cloneReceipt.exitCode !== 0) throw new Error('git clone failed')
    }
    ...
    const worktreeName = `pauli-${safeSegment(job.projectId, 'project')}-${crypto.createHash('sha256').update(job.idempotencyKey).digest('hex').slice(0, 8)}`
    const create = await run(ORCA_BIN, [
      'worktree',
      'create',
      '--repo',
      `path:${repoDir}`,
    ...
    receipt.commandReceipts.push(create)
    if (create.exitCode !== 0 && !/already|exists/i.test(`${create.stdout}\n${create.stderr}`)) {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/v1/jobs/prepare') {
      ...
      return json(res, 200, await prepareJob(job))
    }
    return json(res, 404, { error: 'NotFound' })
  } catch (error) {
    return json(res, 400, {
      error: error instanceof Error ? error.message : 'BadRequest',
    })
  }
})
